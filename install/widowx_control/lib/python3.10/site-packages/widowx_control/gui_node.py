...
from . import armlink
from .poses_tab import PosesTabMixin
from .sensors_tab import SensorsTabMixin
from .theme import (BG, CARD, INK, MUTED, ACCENT, OK, WARN, LINE)
...
# A paleta mudou-se para theme.py quando as abas Sensores e Poses viraram
# modulos proprios (tres copias da mesma paleta divergem na primeira vez que
# alguem acerta um tom so num arquivo).
...
class GuiNode(Node):
    ...
    def __init__(self):
        ...
        # Aba Sensores: porta do touch sensor STM32 ("" = auto-detect) e grade
        # do sensor (4 ou 5), mesma convencao do launch do touch_pack.
        self.touch_port = str(self.declare_parameter(
            "touch_port", "").value or "")
        # INTEIRO, e o launch manda inteiro (ParameterValue com value_type):
        # quem declara um tipo e recebe outro morre na largada. Ja foi string
        # aqui, para "aceitar sensor:=5 como texto", e era o lado errado da
        # conta - o launch_ros infere o tipo do valor e mandava INTEGER.
        self.touch_grid = int(self.declare_parameter("sensor", 5).value or 5)
        self.poses_file = str(self.declare_parameter("poses_file", "").value
                              or "")
        ...
        # ── Aba Sensores ─────────────────────────────────────────────────
        # So o escalar do toque. A celula de carga do touch_pack NAO entra
        # aqui: o WidowX nao tem celula, e o espaco lateral que ela ocupava no
        # cr10twin foi para os graficos (ver o docstring de sensors_tab).
        # Este topico e o caminho alternativo do escalar, para quando o STM32
        # esta noutra maquina e alguem o republica na rede ROS.
        self._sensor_lock = threading.Lock()
        self.touch_value = 0.0
        self.touch_value_ts = 0.0
        self.create_subscription(Float32, "/touch_sensor/value",
                                 self._on_touch_value, qos_profile_sensor_data)
    ...
    # ---------------------------------------------------------- sensores
    # O callback roda na thread do rclpy e a GUI le da thread do Tk: valor e
    # carimbo saem em bloco, sob o mesmo lock em que entraram, para a tela
    # nunca mostrar um valor de agora com o carimbo do anterior (e concluir
    # "sem dado" com dado fresco na mao).
    def _on_touch_value(self, msg):
        with self._sensor_lock:
            self.touch_value = float(msg.data)
            self.touch_value_ts = time.time()

    def touch_scalar(self):
        """(valor, carimbo) do escalar do toque vindo por ROS."""
        with self._sensor_lock:
            return self.touch_value, self.touch_value_ts
...
class WidowXGui(SensorsTabMixin, PosesTabMixin):
    ...
    def __init__(self, node):
        ...
        self.selected = "base"
        self._started = False
        # Ligado enquanto a aba Poses mexe nos sliders por conta propria: sem
        # isto o set() de cada slider marcaria um alvo novo e o laco da GUI
        # republicaria a pose que a execucao acabou de mandar.
        self._suppress_pending = False
        # Mensagem transitoria na barra de status (ver flash/_refresh_status).
        self._flash_text = ""
        self._flash_color = INK
        self._flash_until = 0.0

        self.root = tk.Tk()
        self.root.title("openARM — WidowX Mark II")
        # Um pouco maior que antes: com as abas Poses e Sensores a janela
        # antiga (1020×700) nascia com a sequência e o card da célula
        # espremidos.
        ...
        # As duas abas novas preparam seu estado ANTES de serem montadas: a
        # figura do toque e construida a partir da fonte serial, e a lista de
        # poses e lida do disco.
        self.init_poses(node.poses_file)
        self.init_sensors(node.touch_port, node.touch_grid)
        ...
        # Abre a serial do touch sensor e inicia o laco da aba Sensores.
        self.start_sensors()
    ...
    # ------------------------------------------------------------- helpers
    def _scale_set(self, scale, value):
        """`set()` numa ttk.Scale DESABILITADA e ignorado em silencio (Tk 8.6,
        verificado nesta maquina: set(45) numa scale desabilitada deixa o valor
        anterior). Como os sliders ficam desabilitados justamente quando o
        torque esta solto, era esse silencio que impedia os sliders de
        acompanharem o braco movido a mao - o comportamento que o README do
        pacote descreve. Solta o estado, escreve e devolve."""
        disabled = "disabled" in scale.state()
        if disabled:
    ...
    def _set_sliders_to(self, units):
        for _, joint in SLIDERS:
            ...
            self._scale_set(self.scales[joint],
                            u if joint == "gripper" else units_to_deg(joint, u))
    ...
    def _on_slider(self, joint):
        self._refresh_labels()
        self.active_joint = joint
        if self.active and not self._suppress_pending:
            self.pending = True
    ...
    def _tick(self):
        ...
        if self.active and self.pending:
            self.pending = False
            # Durante uma sequencia de poses quem manda nos alvos e a aba
            # Poses. O alvo pendente e DESCARTADO (nao adiado): publica-lo ao
            # fim da execucao mandaria o braco de volta para onde o slider
            # estava antes, depois de a sequencia ter terminado.
    ...
    def flash(self, text, color=None):
        """Mensagem transitoria na barra de status (6 s).

        As abas novas falam por aqui em vez de abrir caixas de dialogo: numa
        aba onde se captura e reordena pose atras de pose, um messagebox por
        acao custa um clique a mais em cada passo."""
        self._flash_text = str(text)
        self._flash_color = color or INK
        self._flash_until = time.time() + 6.0

    def _refresh_status(self):
        if time.time() < self._flash_until:
            self.status_var.set(self._flash_text)
            self.status_dot.config(fg=self._flash_color)
            return
        text = self.node.status_text
        self.status_var.set(text)
        if "INICIALIZANDO" in text:
            self.status_dot.config(fg=ACCENT)
            self._adopt_driver_state(False)
        elif "ATIVO" in text:
            self.status_dot.config(fg=OK)
            self._adopt_driver_state(True)
    ...
    def _adopt_driver_state(self, driver_ativo):
        """Segue o estado do DRIVER em vez de so o proprio.

        O driver agora entra em backhoe e vai a home sozinho ao ligar (ver
        _boot_to_home). Sem isto a GUI continuaria achando que o braco esta
        inativo e o operador clicaria em Ativar - que reentra em backhoe e
        leva o braco ao neutro DE FABRICA, ou seja, exatamente a pose
        intermediaria que a inicializacao existe para evitar."""
        if driver_ativo == self.active:
            return
        self.active = driver_ativo
        self._set_sliders_enabled(driver_ativo)
        if driver_ativo:
            # Alinha os sliders com onde o braco esta AGORA (a home), senao o
            # primeiro toque num slider mandaria o braco de volta ao neutro
            # que eles mostravam.
            units = {j: armlink.rad_to_units(j, rad)
                     for j, rad in self.node.joint_states.items()
                     if j in armlink.JOINTS}
            self._set_sliders_to(units or self.node.user_neutral)
            self.pending = False
    ...
    def _on_close(self):
        """Fecha a serial do touch sensor e para a execucao de poses antes de
        derrubar a janela: a thread do leitor e a da sequencia sao daemon,
        mas uma sequencia em curso deixaria o braco a caminho de uma pose."""
        self.stop_poses()
        self.stop_sensors()
        self.root.destroy()
