...
PORTE de `touch_pack/touch_source.py` (repositório cr10twin) para o
widowx_control, feito em 27/08/2026. O parser, o estado compartilhado e a
figura são os MESMOS — o sensor é o mesmo STM32 pela mesma USB, e divergir
aqui significaria dois parsers para um firmware só. As únicas mudanças são as
constantes, que passaram a morar neste arquivo em vez de virem do
`constants.py` do outro pacote (ver o bloco abaixo).
"""
...
# ── Constantes vindas de touch_pack/constants.py ───────────────────────────
# No cr10twin isto é um `from .constants import (...)`. Aqui elas moram no
# próprio módulo: o widowx_control não tem (nem deve ganhar) o constants.py
# inteiro do outro pacote, que carrega a cinemática do CR10, a célula Fibos e
# os tópicos da palpação. Só o que o parser e a figura usam veio junto.

# VIDs de chips de ponte USB-serial (conversores RS485). São EXCLUÍDOS do
# auto-detect do touch sensor: um conversor plugado casaria com o filtro de
# /dev/ttyUSB* e a fonte abriria a porta errada.
FT_USB_VIDS = (
...
class TouchSensorSource:
    ...
    def snapshot(self) -> dict:
        ...
        18/08 (ver taxel_frame_to_physical no topo deste módulo). Esta função é a
